...
| `marketing/articles/` | Posts for the store's blog, one directory per handle (body, metadata, recorded image URLs) plus a derived manifest pinning each body's and metadata's hash. **Not shipped**: Shopify renders articles from Admin, and a body is emitted raw, so `articles:check` refuses any markup outside a strict allowlist. The repo is the source of truth; the tooling writes hidden articles only and publishing is a hand action in Admin. See [`marketing/articles/README.md`](marketing/articles/README.md) |
...
|---|---|---|
| `validate` | PR opened / synchronize / reopened (same-repo heads) | Two jobs. `deploy-preview` (skipped for drafts and Dependabot) creates a per-PR unpublished theme `pr-<n>-preview` and comments the link. `validate` (needs `deploy-preview`, runs even when it is skipped or fails) sequentially runs `theme-check`, `reconcile`, the tooling suites (`size-chart`, `blank-inventory`, `sku`, `seo-review`, `site-check`, `applique-grid`, `email-icons`, `notifications` (regeneration check plus tests), `policies` (repo-consistency check plus tests), `articles` (repo-consistency and safety check plus tests), `product-images`, `smoke` deploy smoke-test units, the blank-id guard, the tracked-media guard (fails on any tracked file under `product-images/` or `article-images/`, or any tracked `.heic` anywhere), the SKU tables lint, the catalogue manifest lint, and the contrast lint), `actionlint + shellcheck` (workflow YAML plus the shell inside the composite actions), `zizmor`, `gitleaks`, and a pa11y-ci accessibility audit of the preview theme, plus an aggregator that posts a sticky CI report. The single required check on `main` is `validate / validate`. |
