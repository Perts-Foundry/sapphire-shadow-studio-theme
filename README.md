...
| `sections/password.liquid` | The animated brand gradient on `.section-password .section-background`, and the `background-color` / `color` forced on `.password-dialog` so the dialog does not flash a white full-screen panel over the dark page. |
