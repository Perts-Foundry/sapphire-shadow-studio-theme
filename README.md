...
| `sections/main-policy.liquid` | Rewritten: BEM markup with a scoped `{% stylesheet %}` in place of upstream's `page-width-content` / `layout-panel-flex` wrapper, an explicit 720px measure replacing upstream's undefined `var(--page-width-narrow)`, and a `<policy-nav-component>` shell driven by `assets/policy-nav.js`. None of it renders without `templates/policy.liquid`, which is not an upstream file and cannot be JSON (Shopify refuses a JSON policy template). |
