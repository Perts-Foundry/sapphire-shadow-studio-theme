...
4. Open a PR. Validate runs `theme-check`, `reconcile`, `size-chart`, `blank-inventory`, `blank-id-guard`, `smoke`, `actionlint`, `zizmor --pedantic`, `gitleaks` as one sequential job (each an `npm run <name>:test`-style step; see `validate.yml`). Single required check on `main`: `validate / validate`. To re-run, push a new commit (no comment trigger).
