...
**Social links have one source of truth: `settings.social_*_link` (5 platforms), rendered everywhere via `snippets/social-links.liquid`.** It derives `@handle` from each URL's last path segment; a profile-ID or query-string URL (`.php`, `?`, `=`) fails derivation and silently falls back to the platform name, so keep these URLs in vanity-slug form. `blocks/social-links.liquid`, `blocks/_social-link.liquid`, and `blocks/_footer-social-icons.liquid` are dead upstream leftovers: do not edit or reintroduce them. The set is capped at Facebook/Instagram/TikTok/YouTube/Pinterest; adding a platform is a code change to the snippet, not an editor setting. Rationale: release-notes.md.
