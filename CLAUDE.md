...
3. **Signed-commit gate on auto-deploy paths.** Both `workflow_run` arms of `deploy.yml` assert `verification.verified === true`, `commit.author.login` equal to the expected bot (`shopify[bot]` for shopify-sync; `dependabot[bot]` for dependabot), and `pull_request.user.login` equal to the expected PR opener (`github-actions[bot]` for shopify-sync, since `sync.yml` opens that PR; `dependabot[bot]` for dependabot). The integrity boundary on commit content is `author.login === expectedBot` AND `verification.verified === true`: the bot's GPG/SSH signature is bound to its key, so rewriting commit content invalidates the signature; a forged-content commit cannot pass both checks simultaneously. `commit.committer.login` is allowed to be either the expected bot OR `web-flow` and is **informational, not a security signal** — anyone with `contents: write` can produce a `committer=web-flow` commit via web-UI edits, the Update-branch / Rebase buttons, `PUT /contents/{path}` REST calls, or `@dependabot rebase`/`@dependabot recreate` issue-comment commands, so it cannot be treated as unimpersonable. Accepting both committer values avoids the false-positive failures the earlier strict-equality form produced on every rebased PR; the security property is preserved by author + signature + PR-opener identity. Git-commit-header fields are not consulted (those are forgeable).
