...
- **test-engineer**: theme Liquid has no test framework, so skip it for theme changes. Run it when the code behind any `node --test` suite changes: `scripts/size-chart/`, `scripts/blank-inventory/`, `scripts/applique-grid/`, `scripts/email-icons/`, `scripts/notifications/`, `scripts/catalogue/`, `scripts/lib/`, the top-level `scripts/*.test.mjs`, and `.github/actions/shopify-theme-push/` (the `smoke:test` suites: the post-deploy smoke, the push-rejection audit, the report formatter and the retry helper). `blank-inventory/` writes live inventory and `upload-product-media.mjs` writes live product media, so those two are higher-risk; the `shopify-theme-push` suites are the only automated coverage the live-push path has before a production deploy.
