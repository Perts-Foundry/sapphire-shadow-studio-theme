...
**Do NOT run `npm run policies:push`.** It writes a legal policy on the live store, which is customer-facing and cannot be rolled back by a redeploy. It is operator-only: it refuses when `CI` is set or stdin is not a TTY, requires `--confirm=<type>` matching `--type`, and requires `--expect-live-sha` from its own dry run. Running the dry run (`npm run policies:push -- --type <type>`, no `--confirm`) is read-only and fine, as is `npm run policies:check` (offline) and `npm run policies:pull` (read-only client, refuses any mutation before it reaches the network). To inspect the live theme, pull it read-only to a scratch path: `npx shopify theme pull -s sapphire-shadow-studio --live --path /tmp/live --nodelete`.
...
5. **Shipping copy has five sources of truth and only one now sits outside the repo**: the Admin shipping-rate names. The `/policies/shipping-policy` shop policy moved in as `marketing/policies/shipping_policy.html`; the six product-template accordions defer to it and state no duration at all, held byte-identical by a cohesion test.
