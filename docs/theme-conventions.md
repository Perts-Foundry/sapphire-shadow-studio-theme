...
- **Any link-color rule scoped to a rich-text container must exclude button-classed links:** `a:where(:not(.button, .button-primary, .button-secondary))`, the form `assets/base.css` already uses. Rich text is Admin-authored, so an operator can paste `<a class="button">` into a page body or a shop policy, and a two-class descendant selector (0,2,1) outranks `.button`'s own `color: var(--button-color)` (0,1,0). The label then takes the link color over the button background, which on scheme-1 is `#000000cf` on `#000000`: an unreadable black box. `snippets/policy-page.liquid` shipped that bug. `:where()` keeps the original specificity, so ordinary links are unaffected.
