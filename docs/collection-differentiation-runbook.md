...
`snippets/breadcrumbs.liquid` scans `healthcare`, `the-vitals-collection`, `featured` in order, so a
product leaving `healthcare` gets a different fallback parent. Check the resulting trail is still
the one you want.

## The problem

`featured` and `healthcare` list an identical five products. Both are indexable. Their meta
...
**`healthcare` becomes a real category with a stated rule.** Contents: `lead-ii-crewneck`,
...
least one must be a product `healthcare` does not contain**. `gift-card` is the natural pick,
because it will never belong in `healthcare` and so guarantees the difference in that direction
permanently. Manual and curated, never a rule set.

Result: `healthcare \ featured` is non-empty and `featured \ healthcare` is non-empty. No subset
