...
### `requires_shipping` is the predicate, and the gift-card spelling differs by object

**Whether shipping applies at all, and every sum behind a threshold claim, turns on `requires_shipping`, never on `cart.total_price`.** Checkout excludes digital gift card value from its price-based rate conditions: a gift card never enters a shipment, so the tiers evaluate the shippable merchandise subtotal. This is platform behaviour ([Shopify Help Center: shipping rates](https://help.shopify.com/en/manual/fulfillment/setup/shipping-rates)), and it was verified at live checkout on 2026-08-31 ($65 garment + $50 gift card was charged the flat rate despite the $115 total), so a later re-test that disagrees means a platform change rather than a misread. `snippets/shipping-info.liquid` sums `final_line_price` over `requires_shipping` line items, and `snippets/cart-summary.liquid` gates the whole shipping block on the same predicate. Reading `cart.total_price` promises free shipping that checkout will not honour on a mixed cart, and nothing in CI catches it. A gift-card check decides only **which sentence to show**, never whether shipping applies: that is the one thing `gift_card?` is for at `blocks/price.liquid`, per the subsection above.

**The spelling differs by object, and the two wrong forms are nil rather than errors.** On the **product** drop the property carries a question mark, `product.gift_card?`; plain `product.gift_card` is undefined. On a **line item** it does not, `item.gift_card`; `item.product.gift_card` is undefined. So `product.gift_card` and `item.product.gift_card` are both nil, and so is `item.gift_card?`. Nil is falsy in a condition, which is the actual mechanism: nothing raises, nothing warns, and `theme-check` passes, so the wrong branch is taken silently and forever. Three sites shipped that bug here: `blocks/price.liquid` advertised flat-rate shipping on the gift card page, `snippets/cart-summary.liquid` showed the flat-rate line on a gift-card-only cart, and `snippets/cart-products.liquid` carried a clause that never excluded anything. Rationale: release-notes.md.
