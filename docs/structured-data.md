...
- Product / ProductGroup markup is **not** routed here. It comes from Shopify's `{{ product | structured_data }}` filter and is not extensible. A second Product node will appear beside it once the first Judge.me review is published: the app's rich snippets emit their own. The Judge.me readiness audit (2026-09) weighed three options: suppress the app's node (its Review and AggregateRating are the reason the app is installed), drop the theme's (Shopify's filter carries the offer, variant and merchant data the app's node does not), or keep both and decide from evidence. It took the middle path: both stay, `seo-review` now reports `jsonld-product-duplicate` as the one-time signal, and the revisit trigger is the first published review, when the crawl and Google's Rich Results test on that product page show which node Google reads and whether the two disagree. The two candidate owners are the theme's `structured_data` output and the app's rich snippets; the operator picks from that evidence. The directive (what not to do in the meantime) lives in `.claude/rules/structured-data.md` and is not repeated here.
