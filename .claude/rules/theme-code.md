...
- **Never iterate `product.variants` for completeness** (pickers, availability maps, hidden selects, JSON dumps): Liquid caps it at 250 entries, the Lead II products carry 288, and the loop truncates with no error. Use `options_with_values`, `product_option_value.available` / `.variant` / `.id`, `product.variants_count`, and a section render with `option_values` (the pattern in `docs/theme-conventions.md`). `.first`, `.size` and `variants_count` remain fine. The audit that found the one offender: `release-notes.md`.
