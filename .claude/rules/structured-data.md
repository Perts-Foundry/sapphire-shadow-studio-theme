...
**Product markup is still not routed here; this rule governs only the response to app-emitted Product nodes.** An installed app (Judge.me rich snippets at the time of writing) may emit its own Product node beside the theme's `structured_data` output once a review is published. When `seo-review` reports `jsonld-product-duplicate`, that is the anticipated overlap, not a regression. Do not resolve it by editing the app block or app embed, adding Liquid that suppresses app output, deleting the theme's Product node, or silencing the check. The only sanctioned action is the `TODO.md` decision item, which the operator decides from evidence (the crawl plus the Rich Results test). Rationale: `release-notes.md`.
