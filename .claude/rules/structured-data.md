...
**This snippet does not emit Product or ProductGroup markup, and this rule does not change that; it governs only how to respond when an app also emits a Product node beside Shopify's own.** An installed app (Judge.me rich snippets at the time of writing) may emit its own Product node beside the theme's `structured_data` output once a review is published. When `seo-review` reports `jsonld-product-duplicate`, that is the anticipated overlap, not a regression. Do not resolve it by editing, removing or disabling the app block or app embed, adding Liquid that suppresses app output, deleting the theme's Product node, modifying either node's fields so the two no longer read as duplicates, or silencing the check. The only sanctioned action is the `TODO-list.md` decision item, which the operator decides from evidence (the crawl plus the Rich Results test). Rationale: `release-notes.md`.
