...
3. Run the `sync` per-id loop from its step 3 with the scratch file as the paste source: clipboard,
   paste, byte check against the scratch file, Preview, `verify-render.mjs --dump <file> --id <id>
   --version <target> --manifest <scratch>/manifest.json`, Save, reload, re-verify, `state.mjs
   seen` with the target sha and ref.
