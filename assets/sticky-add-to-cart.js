...
class StickyAddToCartComponent extends Component {
  ...
  connectedCallback() {
    ...
    const trigger = this.dataset.trigger || 'on_scroll';
    if (trigger === 'always') {
      this.#setupAlwaysVisible();
    } else if (trigger === 'pixels') {
  ...
  #setupIntersectionObserver() {
    ...
    this.#mainBottomObserver = new IntersectionObserver(
      ...
      {
        rootMargin: `${parseInt(this.dataset.footerOffset || '200')}px 0px 0px 0px`,
      }
    );
  ...
  /**
   * Shows the sticky bar immediately and hides it near the footer.
   * Used when trigger mode is 'always'.
   */
  #setupAlwaysVisible() {
    this.#showStickyBar();
    ...
    const footerOffset = parseInt(this.dataset.footerOffset || '200');
    this.#mainBottomObserver = new IntersectionObserver(
      (entries) => {
        const [entry] = entries;
        if (!entry) return;
      ...
      { rootMargin: `${footerOffset}px 0px 0px 0px` }
    );
    this.#mainBottomObserver.observe(footer);
  }

  /**
   * Shows the sticky bar after the page has been scrolled a set pixel distance.
   * Used when trigger mode is 'pixels'.
   */
  #setupScrollObserver() {
    const threshold = parseInt(this.dataset.scrollThreshold || '300');
    const footer = document.querySelector('footer') ?? document.querySelector('[class*="footer-group"]');
    const footerOffset = parseInt(this.dataset.footerOffset || '200');
    const { signal } = this.#abortController;
    ...
    window.addEventListener('scroll', checkScroll, { signal, passive: true });
    checkScroll();
